/* ==========================================================================
   ...
   Handles Auth, State, Routing, File Uploads, Privacy Syncing, and Rendering
   ========================================================================== */
...
document.getElementById('login-form').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        if (result.error) {
            ...
            appState.actualRole = result.role; appState.role = result.role; 
            appState.actualUser = result.userProfile; appState.currentUser = result.userProfile; 
            appState.authString = pass; appData = result.data; 
            
            if(!appData.materials) appData.materials = [];
            if(!appData.certificates) appData.certificates = [];
            
            if (remember) { localStorage.setItem("ei_role", result.role); localStorage.setItem("ei_auth", pass); }
...
document.getElementById('pin-login-form').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        if (result.error) {
            hideLoader(); document.getElementById('pin-error').innerText = result.error;
            document.getElementById('pin-error').classList.remove('hidden-initially'); 
            document.getElementById('pin-input').value = ''; 
        } else {
            appState.actualRole = result.role; appState.role = result.role; appData = result.data; 
            if(!appData.materials) appData.materials = []; if(!appData.certificates) appData.certificates = [];
            setupApplicationUI(); hideLoader();
        }
    } catch (error) { hideLoader(); document.getElementById('pin-error').innerText = "Network Error."; document.getElementById('pin-error').classList.remove('hidden-initially'); }
});


// ==========================================================================
// 5. UI ROUTING & SECURE PERSPECTIVE SWITCHER
// ==========================================================================
function setupApplicationUI() {
    ...
    } else { btnAdminView.classList.add('hidden-initially'); }

    // Hard UI Blocks based on Role
...
function switchTab(tabId) {
    // SECURITY: Force Students out of admin views if they try to click Bottom Nav
    if (appState.role === 'student' && ['dashboard', 'students', 'analytics', 'admission', 'job', 'print', 'expense', 'uploads'].includes(tabId)) {
        tabId = 'student-dash';
    }
    ...
    setTimeout(() => {
        ...
        if(titles[tabId]) {
            ...
            // If student clicks Home (nav 0), ensure the home button stays active
            if(appState.role === 'student' && tabId === 'student-dash') document.querySelectorAll('#bottom-nav .nav-btn')[0].classList.add('active');
        }
...
function changeAdminView(targetRole) {
    ...
    } else { appState.currentUser = appState.actualUser; }
    
    appState.role = targetRole;
    closeViewSwitcher(); document.getElementById('admin-view-banner').classList.remove('hidden-initially'); setupApplicationUI();
}

function exitAdminPreview() {
    appState.role = appState.actualRole; appState.currentUser = appState.actualUser;
    document.getElementById('admin-view-banner').classList.add('hidden-initially'); setupApplicationUI();
}
...
async function syncDatabase(actionDesc) {
    if (appState.actualRole !== 'admin') { alert("Action Denied: You do not have Write Permissions."); return false; }
    
    showLoader();
    try {
        const response = await fetch(GAS_URL, { method: 'POST', headers: { "Content-Type": "text/plain;charset=utf-8" }, body: JSON.stringify({ password: "admin", data: appData }) });
        const result = await response.json();
        hideLoader();
        if (result.error) { alert("Server Sync Error: " + result.error); return false; }
        return true;
    } catch (error) { hideLoader(); alert("Network error. Database sync failed."); return false; }
}


// ==========================================================================
// 7. SECURE NOTIFICATIONS & MODALS
// ==========================================================================
function checkDuesSilently() {
    let showDot = false;
    if (appState.role === 'admin' || appState.role === 'staff') {
        appData.students.forEach(st => { if((st.totalFee - getDynamicPaidFee(st)) > 0) showDot = true; });
    } else if (appState.role === 'student' && appState.currentUser) {
        if ((appState.currentUser.totalFee - getDynamicPaidFee(appState.currentUser)) > 0) showDot = true;
        if (appData.notices && appData.notices.length > 0) showDot = true;
    }
    
    const dot = document.getElementById('notif-dot');
    if(showDot) dot.classList.remove('hidden-initially'); else dot.classList.add('hidden-initially');
}
...
function openNotificationModal() {
    ...
    // STUDENT VIEW (Private Dues + Alerts)
    } else if (appState.role === 'student' && appState.currentUser) {
        const dues = appState.currentUser.totalFee - getDynamicPaidFee(appState.currentUser);
        if (dues > 0) {
            listEl.innerHTML += `
                <div style="background: var(--danger-bg); padding: 18px; border-radius: 20px; margin-bottom: 20px; border: 1px solid rgba(244, 63, 94, 0.2);">
                    <span style="font-size: 0.7rem; font-weight: 900; color: var(--danger); text-transform: uppercase;"><i class="fa-solid fa-triangle-exclamation mr-1"></i> Your Pending Dues</span>
                    <h2 style="font-size: 1.8rem; font-weight: 900; color: var(--danger); margin-top: 4px; margin-bottom: 4px;">₹${dues}</h2>
                    <p style="font-size: 0.75rem; font-weight: 700; color: var(--text-main);">Please clear your balance at the office to avoid interruptions.</p>
                </div>`;
        } else {
            listEl.innerHTML += `
                <div style="background: var(--success-bg); padding: 16px; border-radius: 20px; margin-bottom: 20px; border: 1px solid rgba(16, 185, 129, 0.2); display: flex; align-items: center;">
                    <i class="fa-solid fa-circle-check text-3xl text-emerald-500 mr-4"></i>
                    <div><h4 style="font-size: 1.1rem; font-weight: 900; color: var(--success);">All Clear</h4><p style="font-size: 0.75rem; font-weight: 700; color: var(--text-main);">You have no pending dues.</p></div>
                </div>`;
        }

        listEl.innerHTML += `<h4 class="section-heading"><i class="fa-solid fa-bullhorn mr-2"></i> Institute Alerts</h4>`;
        if(appData.notices && appData.notices.length > 0) {
            appData.notices.forEach(n => {
                let mediaHtml = '';
                if (n.media) {
                    if (n.mediaType && n.mediaType.startsWith('video/')) mediaHtml = `<video src="${n.media}" controls style="width: 100%; border-radius: 12px; margin-top: 10px;"></video>`;
                    else mediaHtml = `<img src="${n.media}" style="width: 100%; border-radius: 12px; margin-top: 10px;">`;
                }
                listEl.innerHTML += `
                    <div style="background: #f8fafc; padding: 16px; border-radius: 20px; border: 1px solid #e2e8f0; margin-bottom: 12px;">
                        <h4 style="font-size: 0.95rem; font-weight: 900; color: var(--primary);">${n.title}</h4>
                        <p style="font-size: 0.8rem; font-weight: 600; color: var(--text-main); margin-top: 6px;">${n.message}</p>
                        ${mediaHtml}
                        <p style="font-size: 0.65rem; font-weight: 800; color: var(--text-muted); margin-top: 10px; text-transform: uppercase;">${n.date}</p>
                    </div>`;
            });
        } else { listEl.innerHTML += `<p style="text-align: center; color: var(--text-muted); font-size: 0.8rem; font-weight: 700; padding: 12px 0;">No recent alerts.</p>`; }
    }
...
function openHistoryModal(type) {
    ...
    if (filtered.length === 0) {
        ...
        filtered.slice().reverse().forEach(tx => {
            const isInc = tx.type === 'income'; const color = isInc ? 'var(--success)' : 'var(--danger)'; const sign = isInc ? '+' : '-';
            container.innerHTML += `<div class="list-item" style="padding: 14px 18px; margin-bottom: 8px;"><div class="list-info"><h4 style="font-size: 0.95rem;">${tx.title.replace('Job Desk: ','').replace('Print Desk: ','')}</h4><p style="font-size: 0.7rem;">${tx.date}</p></div><div class="list-value"><h3 style="color: ${color}; font-size: 1.15rem;">${sign}₹${tx.amount}</h3></div></div>`;
        });
    }
...
function openStudentDetailModal(studentId) {
    ...
    document.getElementById('detail-name').innerText = st.name; document.getElementById('detail-id').innerText = `ID: ${st.id}`;
    document.getElementById('detail-phone').innerText = st.phone; document.getElementById('detail-date').innerText = st.date;
    document.getElementById('detail-course').innerText = st.course; document.getElementById('detail-total').innerText = `₹${st.totalFee}`;
    document.getElementById('detail-paid').innerText = `₹${actualPaid}`; document.getElementById('detail-due').innerText = `₹${dues}`;

    const historyContainer = document.getElementById('detail-history-list'); historyContainer.innerHTML = '';
    ...
    if (stuTx.length === 0) {
        ...
        stuTx.slice().reverse().forEach(tx => { historyContainer.innerHTML += `<div class="list-item" style="padding: 10px 16px; margin-bottom: 6px;"><div class="list-info"><h4 style="font-size:0.8rem;">Fee Payment</h4><p style="font-size:0.65rem;">${tx.date}</p></div><div class="list-value"><h3 style="color: var(--success); font-size: 1rem;">+₹${tx.amount}</h3></div></div>`; });
    }

    const editBtn = document.getElementById('btn-edit-student');
    if (appState.actualRole === 'admin') { editBtn.classList.remove('hidden-initially'); editBtn.onclick = () => alert("Student Edit feature coming in next update!"); } 
    else { editBtn.classList.add('hidden-initially'); }

    closeNotificationModal(); setTimeout(() => { document.getElementById('modal-student-detail').classList.add('active'); }, 100);
}
...
document.getElementById('upload-material-form').addEventListener('submit', async function(e) {
    ...
    const title = document.getElementById('mat-title').value; const course = document.getElementById('mat-course').value; const fileInput = document.getElementById('mat-file');
    if(!fileInput.files[0]) return alert("Select a file.");
    
    showLoader();
    try {
        const base64Data = await fileToBase64(fileInput.files[0]);
        appData.materials.unshift({ id: 'MAT' + Date.now(), title: title, course: course, filename: fileInput.files[0].name, file: base64Data, date: new Date().toLocaleDateString('en-IN') });
        if(await syncDatabase("Upload Material")) { alert("Study Material Published!"); this.reset(); document.getElementById('mat-file-name').innerText = "Choose File..."; }
...
document.getElementById('upload-cert-form').addEventListener('submit', async function(e) {
    ...
    const studentId = document.getElementById('cert-student').value; const fileInput = document.getElementById('cert-file');
    if(!studentId || !fileInput.files[0]) return alert("Select student and file.");
    
    showLoader();
    try {
        const base64Data = await fileToBase64(fileInput.files[0]);
        appData.certificates.unshift({ id: 'CRT' + Date.now(), studentId: studentId, filename: fileInput.files[0].name, file: base64Data, date: new Date().toLocaleDateString('en-IN') });
        if(await syncDatabase("Issue Certificate")) { alert("Certificate Issued Successfully!"); this.reset(); document.getElementById('cert-file-name').innerText = "Choose File..."; }
    } catch(err) { hideLoader(); alert("Error processing file."); }
});

async function sendBroadcast() {
    if(appState.actualRole !== 'admin' && appState.actualRole !== 'staff') return alert("Action Denied.");
    
    const title = document.getElementById('bc-title').value; const msg = document.getElementById('bc-msg').value; const fileInput = document.getElementById('bc-media');
    if(!title || !msg) return alert("Enter both title and message.");
    
    showLoader();
    try {
        let mediaBase64 = null; let mediaType = null;
        if (fileInput.files[0]) { mediaBase64 = await fileToBase64(fileInput.files[0]); mediaType = fileInput.files[0].type; }

        appData.notices.unshift({ title: title, message: msg, date: new Date().toLocaleDateString('en-IN'), media: mediaBase64, mediaType: mediaType });
        
        if(await syncDatabase("Broadcast Notice")) {
            alert("Notice broadcasted!"); document.getElementById('bc-title').value = ''; document.getElementById('bc-msg').value = ''; fileInput.value = ''; document.getElementById('bc-media-name').innerText = 'Attach Photo/Video (Optional)';
        }
...
document.getElementById('admission-form').addEventListener('submit', async function(e) {
    ...
    const name = document.getElementById('reg-name').value; const phone = document.getElementById('reg-phone').value; const course = document.getElementById('reg-course').value;
    const totalFee = parseFloat(document.getElementById('reg-fee').value); const advancePaid = parseFloat(document.getElementById('reg-paid').value); const dateStr = document.getElementById('reg-date').value;
    ...
    if (advancePaid > 0) appData.transactions.push({ id: 'TXN'+Math.floor(Math.random()*90000+10000), type: "income", title: `Admission - ${name} [${stId}]`, amount: advancePaid, date: dateStr });
    
    recalcStats(); populatePreviewLists(); 
    if(await syncDatabase("Register Student")) {
        alert(`Success!\nStudent Password: EI${name.split(' ')[0]}${dateStr.substring(0,4)}`);
        this.reset(); document.getElementById('reg-date').value = new Date().toISOString().split('T')[0]; switchTab('students');
    }
});
...
['job', 'print', 'expense'].forEach(type => {
    ...
    if(form) {
        form.addEventListener('submit', async function(e) {
            ...
            recalcStats(); if(await syncDatabase(`Save ${type} Entry`)) { this.reset(); renderList(type); }
        });
    }
});

// ==========================================================================
// 10. DYNAMIC RENDERERS (With Student Privacy Integration)
// ==========================================================================
...
function renderStudents() {
    ...
    filtered.forEach(st => {
        const actualPaid = getDynamicPaidFee(st); const dues = st.totalFee - actualPaid; const dueColor = dues > 0 ? 'var(--danger)' : 'var(--success)';
...
function renderList(type) {
    ...
    f.slice().reverse().slice(0, 15).forEach(tx => {
        ...
        listEl.innerHTML += `<div class="list-item" style="cursor: default; pointer-events: none;"><div class="list-info"><h4>${tx.title.replace('Job Desk: ','').replace('Print Desk: ','')}</h4><p>${tx.date}</p></div><div class="list-value"><h3 style="color: ${isInc ? 'var(--success)' : 'var(--danger)'}">${isInc ? '+' : '-'}₹${tx.amount}</h3></div></div>`;
    });
}
...
function renderStudentDashboard() {
    ...
    // --- Dynamic Quick Action Buttons for Students ---
    const dashContainer = document.getElementById('view-student-dash');
    if (!document.getElementById('stu-quick-actions')) {
        const qaHtml = `
        <div id="stu-quick-actions" class="quick-actions" style="grid-template-columns: repeat(3, 1fr); margin-top: 20px;">
            <button onclick="document.getElementById('stu-materials').scrollIntoView({behavior:'smooth'})" class="action-btn"><div class="icon-circle" style="color: #6366f1; background: #e0e7ff;"><i class="fa-solid fa-book-open"></i></div><span>Materials</span></button>
            <button onclick="document.getElementById('stu-certificates').scrollIntoView({behavior:'smooth'})" class="action-btn"><div class="icon-circle" style="color: #10b981; background: #d1fae5;"><i class="fa-solid fa-award"></i></div><span>Certificates</span></button>
            <button onclick="document.getElementById('stu-notices').scrollIntoView({behavior:'smooth'})" class="action-btn"><div class="icon-circle" style="color: #f59e0b; background: #fef3c7;"><i class="fa-solid fa-bullhorn"></i></div><span>Notices</span></button>
        </div>`;
        const statGrid = dashContainer.querySelector('.stat-grid');
        statGrid.insertAdjacentHTML('afterend', qaHtml);
    }

    document.getElementById('stu-name').innerText = st.name; document.getElementById('stu-course').innerText = st.course; document.getElementById('stu-date').innerText = st.date;
    const avatarEl = document.getElementById('stu-avatar');
    if (st.image) { avatarEl.innerHTML = `<img src="${st.image}" style="width: 100%; height: 100%; object-fit: cover; border-radius: 50%;">`; avatarEl.style.padding = "0"; } 
    else { avatarEl.innerHTML = st.name.charAt(0).toUpperCase(); avatarEl.style.padding = ""; }
    
    const actualPaid = getDynamicPaidFee(st); const dues = st.totalFee - actualPaid; 
    document.getElementById('stu-due-amount').innerText = `₹${dues}`; document.getElementById('stu-due-card').style.background = dues > 0 ? 'var(--danger-bg)' : 'var(--success-bg)'; document.getElementById('stu-due-amount').style.color = dues > 0 ? 'var(--danger)' : 'var(--success)';

    const certCtn = document.getElementById('stu-certificates');
    const myCerts = (appData.certificates || []).filter(c => c.studentId === st.id);
    if(myCerts.length > 0) {
        certCtn.innerHTML = myCerts.map(c => `<div class="list-item" style="padding: 16px;"><div class="flex items-center"><i class="fa-solid fa-award text-2xl text-emerald-500 mr-4"></i><div><h4 style="font-weight: 800; font-size: 0.9rem;">Course Certificate</h4><p style="font-size: 0.7rem; color: var(--text-muted);">${c.date}</p></div></div><a href="${c.file}" download="${c.filename}" class="btn-primary" style="width: auto; padding: 10px 16px; font-size: 0.8rem; background: var(--success);"><i class="fa-solid fa-download"></i></a></div>`).join('');
    ...
    if(myMats.length > 0) {
        matCtn.innerHTML = myMats.map(m => `<div class="list-item" style="padding: 16px;"><div class="flex items-center"><i class="fa-solid fa-file-pdf text-2xl text-indigo-500 mr-4"></i><div style="max-width: 160px;"><h4 style="font-weight: 800; font-size: 0.9rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${m.title}</h4><p style="font-size: 0.7rem; color: var(--text-muted);">${m.date}</p></div></div><a href="${m.file}" download="${m.filename}" class="btn-primary" style="width: auto; padding: 10px 16px; font-size: 0.8rem;"><i class="fa-solid fa-download"></i></a></div>`).join('');
    ...
    if(appData.notices && appData.notices.length > 0) {
        appData.notices.forEach(n => {
            let mediaHtml = '';
            if (n.media) {
                if (n.mediaType && n.mediaType.startsWith('video/')) mediaHtml = `<video src="${n.media}" controls style="width: 100%; border-radius: 12px; margin-top: 8px; box-shadow: 0 4px 10px rgba(0,0,0,0.1);"></video>`;
                else mediaHtml = `<img src="${n.media}" style="width: 100%; border-radius: 12px; margin-top: 8px; box-shadow: 0 4px 10px rgba(0,0,0,0.1);">`;
            }
            noticesEl.innerHTML += `<div style="border-bottom: 1px solid var(--border-light); padding-bottom: 16px; margin-bottom: 16px;"><h4 style="font-size: 0.95rem; font-weight: 900; color: var(--primary);">${n.title}</h4><p style="font-size: 0.8rem; font-weight: 600; color: var(--text-main); margin-top: 4px;">${n.message}</p>${mediaHtml}<p style="font-size: 0.65rem; font-weight: 800; color: var(--text-muted); margin-top: 8px; text-transform: uppercase;">${n.date}</p></div>`;
        });
...
function renderChart() {
    ...
    let incMap = {}, expMap = {};
    appData.transactions.forEach(tx => { let m = tx.date.substring(0,7); if(tx.type==='income') incMap[m] = (incMap[m]||0) + parseFloat(tx.amount); if(tx.type==='expense') expMap[m] = (expMap[m]||0) + parseFloat(tx.amount); });
    let labels = Object.keys(incMap).concat(Object.keys(expMap)).filter((v,i,a)=>a.indexOf(v)===i).sort().slice(-6);
    analyticsChartInstance = new Chart(document.getElementById('analyticsChart').getContext('2d'), { type: 'bar', data: { labels: labels.length ? labels : ['No Data'], datasets: [ { label: 'Income', data: labels.map(l=>incMap[l]||0), backgroundColor: '#10b981', borderRadius: 6 }, { label: 'Expense', data: labels.map(l=>expMap[l]||0), backgroundColor: '#f43f5e', borderRadius: 6 } ] }, options: { responsive: true, maintainAspectRatio: false, scales: { x: { grid: { display: false } }, y: { beginAtZero: true, border: {dash: [4, 4]} } }, plugins: { legend: { labels: { font: { family: "'Plus Jakarta Sans', sans-serif", weight: 'bold' } } } } } });
}
